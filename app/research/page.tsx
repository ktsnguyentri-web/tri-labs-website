...
import { getAllResearchInsights, ResearchData } from "@/lib/cms";
...
function GridItem({ item }: { item: ResearchData }) {
  return (
    <Link 
      key={item.slug} 
        ...
        <div className="w-full max-w-[1440px] mx-auto px-10 pt-40 pb-24 flex flex-col gap-32">
          ...
          {researchArticles.length > 0 && (
            <div className="w-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {researchArticles.map(item => <GridItem key={item.slug} item={item} />)}
              </div>
            </div>
          )}
          ...
          {toolArticles.length > 0 && (
            <div className="w-full scroll-mt-[100px]" id="tool">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {toolArticles.map(item => <GridItem key={item.slug} item={item} />)}
              </div>
            </div>
          )}
